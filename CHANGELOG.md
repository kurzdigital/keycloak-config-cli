...
### Added
- Add support for Keycloak 26.7. The default build now targets Keycloak 26.7.3 (server version property, `default-keycloak-version` profile, `.env`, and Docker build arg), normalization baselines for 26.7.0, 26.7.2 and 26.7.3 are included, and 26.7.2 and 26.7.3 are covered by the CI matrix.
...
### Fixed
- Fix the import schema rejecting realm fields that a Keycloak realm export actually emits. The schema sets `additionalProperties: false`, so every unlisted key fails validation: the WebAuthn policy block listed 14 of 26 fields (missing, on both the two-factor and the passwordless side, `attestationConveyancePreference`, `authenticatorAttachment`, `requireResidentKey`, `residentKey`, `userVerificationRequirement` and `extraOrigins`, plus `passkeysEnabled` and `mediation`), and `maxSecondaryAuthFailures`, `otpPolicyCodeReusable`, `otpSupportedApplications`, `localizationTexts`, `verifiableCredentialsEnabled` and `scimApiEnabled` were absent entirely. Also removes a duplicate `loginTheme` declaration.
